...
function EditarRenta({ rentas, setRentas }) {
  ...
  const cambiarDatosRenta = (evento) => {
    ...
    setDatosRenta((datosAnteriores) => ({ ...datosAnteriores, [name]: type === "checkbox" ? checked : value, estatus: name === "estatus" ? value === "ENTREGADA" : false }));
  };
  ...
  const modificarRenta = () => {
    ...
    const nuevasRentas = rentas.map((renta) => renta.idRentar === idRentar ? rentaModificada : renta);
    setRentas(nuevasRentas);
    setRentaModificada(true);
  };

  const manejarEnvio = (evento) => { evento.preventDefault(); modificarRenta(); };
  const manejarOkClick = () => { navegar("/renta"); };
  const fechaFormateada = (fecha) => { const fechaDate = new Date(fecha); fechaDate.setDate(fechaDate.getDate() + 1); return fechaDate.toLocaleDateString(); };
    ...
    <div className="EditarRenta">
      ...
      {!rentaSeleccionada && (
        <div className="mensaje">
          <div className="caja">
            <br /><label htmlFor="rentaSelect">Selecciona una renta para modificar</label><br />
            <div className="custom-select-container">
              <select className="custom-select" id="rentaSelect" onChange={seleccionarRenta}>
                <option value="">Selecciona una renta</option>
                {rentas.map((renta) => (
                  <option key={renta.idRentar} value={renta.idRentar}>{renta.idRentar} - {fechaFormateada(renta.fecha_renta)}</option>
                ))}
              </select>
            </div>
            <div className="section">
              <ul className="botC">
                <Link to="/renta"><button>Regresar</button></Link>
              </ul>
            </div>
          </div>
        </div>
      ...
      {rentaModificada && (
        <div className="Cajita-container">
          <div className="Cajita">
            ...
            <div className="section">
              <ul className="botC"><button onClick={manejarOkClick}>Regresar</button></ul>
            </div>
          </div>
        </div>
      )}
          ...
          <form onSubmit={manejarEnvio}>
            <div className="Pedir-container">
              <div className="Pedir">
                ...
                <br />
                <div className="section">
                  <ul className="botC"><button type="submit" disabled={JSON.stringify(rentaSeleccionada) === JSON.stringify(datosRenta)}>Guardar Cambios</button></ul>
                </div>
                <div className="section">
                  <ul className="botC"><Link to="/renta"><button>Regresar</button></Link></ul>
                </div>
              </div>
            </div>
          </form>
